...
public class ForceMergeActionIT extends ESIntegTestCase {
    ...
    public void testDataStreamWithoutCodec() {
        ...
        indexDocsAndRollover(dataStream);
     ...
     */
    private void indexDocsAndRollover(String targetName) {
        for (int i = 0; i < 2; i++) {
    ...
    private String awaitForceMergedIndexName(String sourceIndexName) {
        ...
        awaitClusterState(state -> {
            final var foundIndex = state.metadata()
                ...
                .keySet()
                .stream()
                .filter(i -> i.startsWith(ForceMergeAction.FORCE_MERGE_INDEX_PREFIX) && i.endsWith(sourceIndexName))
                .findFirst();
    ...
    /**
     * Waits until the given index has completed the ILM phase.
     */
    private static void awaitStep(String indexName) {
        awaitClusterState(state -> {
            ...
            return "hot".equals(executionState.phase())
                && PhaseCompleteStep.NAME.equals(executionState.action())
                && PhaseCompleteStep.NAME.equals(executionState.step());
        });
    }
}
